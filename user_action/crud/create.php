...
if (!empty($_POST)) {
    ...
    /*
      ...
      $account_typeError = 'Please enter Account Type';
      $valid = false;
      }
     */
    if (empty($password)) {
    ...
    if ($valid) {
        ...
        $sql = "INSERT INTO users (name,lastname,mail,accounttype,password,money) values(?, ?, ?, ?, ?, ?)";
        $q = $pdo->prepare($sql);
        $q->execute(array($name, $lastname, $mail, $accounttype, $password, $money));
        Database::disconnect();
